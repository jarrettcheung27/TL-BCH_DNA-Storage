...
def plot_oligo_number_distribution(dnas):
    N = len(dnas)
    rNs = [dna['num'] for dna in dnas]#cpoies num of each sequences
    zN = np.sum(np.array(rNs)==0)#没有合成成功的数目
    label = f'{zN}({round(zN/N*100,3)}%) lost' + f'\n{round(sum(rNs)/len(rNs),2)} copies'#损失的比例及副本与合成深度
...
def plot_error_distribution(dnas, th = 1):
    error_nums = error_distribution(dnas)
#     print('error_number:',error_nums,'\n','length:',len(error_nums))
    #===========================
    # print('Log[analysis.plot_error_distribution.error_nums]:\n',error_nums,'\n')
    #===========================    
    esn = sum([n >= th for n in error_nums])#错误容限为1：esn：错误碱基数大于等于一个的序列
    ...
    plt.xlabel('Error nucleotide number of each oligo after voting')
    plt.ylabel('Frequency')
    return esn
...
def error_distribution(dnas):
    error_nums = []
    for dna in dnas:
        if dna['num'] == 0: continue#if copies number = 0，error_numbers is empty
        error_num = dna_chunk(dna).voting_error()
#         print('Log[Syhthesis.analysis.error_distribution.error_num]:',error_num)
        error_nums.append(error_num)
#     print('Log[Syhthesis.analysis.error_distribution.dnas[0]]:\n',dnas[0],'\n')
#     print('Log[Syhthesis.analysis.error_distribution.error_nums[0]]:',error_nums[0],'\n')
    return error_nums

#write voting result into re_dna file,discard other oligo copies.
...
class dna_chunk:
    def __init__(self,dna_in,env = 'jupyter'):
        ...
        self.re_dnas = deepcopy(dna_in['re'])#copy
        self.rN = dna_in['num']#合成的DNA的副本的总数
    ...
    def plot_re_dnas_html(self):
        ...
        for re_dna in self.re_dnas:
            num,error,dna = re_dna#分别把每一个DNA序列的错误图样数目，错误位置，及错误DNA取出
    ...
    def vote(self):
        ...
        # compute subs
        for re_dna in self.re_dnas:
            read_num, error, _ = re_dna#read_num 每一种错误图样的拷贝数
            for e in error:#遍历每一个错误
                pos,tp,base = e#分别取出错误位置，类型，和错误碱基
                if tp == 's':
        ...
        for i in range(len(self.ori_dna)):
            ...
            r = R[i]
            #===========================
            # print(f'Log[analysis.voting_error.R[{i]]:\n',R[i],'\n')
            #===========================                    
            r[base] = self.rN - sum([r[b] for b in self.BASE])#计算这个位置没有被替换的碱基的数目
        #===========================
        # print('Log[analysis.voting_error.r]:\n',r,'\n')
        #===========================               
        # num to prob
        for r in R:
    ...
    def voting_error(self):
        ...
        error_num = 0 
        #===========================
        # print('Log[analysis.voting_error.self.ori_dna]:\n',self.ori_dna,'\n')
        #===========================              
        for i in range(len(self.ori_dna)):
            comp = self.R[i]#在第i个位置，不同 copies之间ATCG的比例，根据这个比例决定最后voting的结果
            ori_base = self.ori_dna[i]#原始droplet在i位置的碱基类型
            for base in ['A','T','C','G']:
                if base is not ori_base:
                    #===========================
                    # if i == 3:
                    #     print('Log[analysis.voting_error.ori_base]:\n',ori_base,'\n')
                    #     print('Log[analysis.voting_error.base]:\n',base,'\n')
                    #===========================                       
                    if comp[base] > comp[ori_base]:#如果最后得到的voting的结果与原始的碱基类型不一致，那么error number+1
                        error_num += 1                        
                        break
                         
#         print('Log[Syhthesis.analysis.dna_chunk.slef.R[0]]:\n',self.R[0],'\n')
        return error_num #error_num 是每一条序列所有副本的错误之和
    
    def voting_result(self): #将出现频率最高的碱基投票出来，然后一个一个接上
