...
#         print('type of self.data:',type(self.data))#self.data is a list
#         print('length of self.data:',len(self.data))
#         print('type of self.data[1]:',type(self.data[1]))#self.data[1] is hexadecimal digits
#         print('type of self.data[1][0]:',type(self.data[1][0]))#self.data[1][0] is int
#         print('chunl_size meaning,self.data: ',self.data[1])
#         print('chunl_size meaning,length of \'self.data[1]\': ',len(self.data[1]))
#         print('chunl_size meaning,self.data[1][0]: ',self.data[1][0])
#         print('chunl_size meaning,self.pad: ',self.pad)
    ...
    def encode(self):
        self.f = DNAFountain(self.data, self.alpha, rs = self.rs_length)#chunks data encoded to dna by dna fountain
#         print('type of self.f:',type(self.data))#self.data is a list
#         print('self.f:',self.f)#self.data is a list
#         print('length of self.f:',len(self.f))
#         print('type of self.f[1]:',type(self.f[1]))#self.data[1] is hexadecimal digits
        ...
        self.f.save(self.encode_path)#output encoded data in a .dna file

    def simu(self, save = True):
        in_dnas = load_dna(self.encode_path)#load  data from .dna file
        out_dnas = self.model(in_dnas, print_state = False)#simulate the process of data go through a dna channel and generate output data.
        loss_num, fail_num, mis_judge_num, ignore_index = error_profile(out_dnas)#error analysis
        self.loss_nums.append(loss_num)
        self.fail_nums.append(fail_num)
        if save: save_simu_result(out_dnas, self.simu_path, ignore_index)#save dna data after simulation in simu_xxx.dna file
        self.out_dnas = out_dnas
        return out_dnas
    
    def decode(self, save = True):
        self.g = Glass(self.simu_path, len(self.data), rs = self.rs_length)#load simu.dna file
        
#         print('type of self.g:',type(self.g))
#         print('self.g:',self.g)#self.g is a list
        ...
        if flag == -1: 
            ...
            print('Success!')
#             print('Log[Fountian_analyzer.FT_analyzer.self.glass.chunks]:',self.g.chunks)
#             for i in range(len(self.g.chunks)):
#                 print('Log[Fountian_analyzer.FT_analyzer.self.glass.chunk_len]:',len(self.g.chunks[i]))
            if save: self.g.save(self.simu_decode_path)#decode dna file back to image file
            return line - errors#return successful lines number
    
    def run(self, plot_res = False):
        out_dnas = self.simu()#execute simu（）
        line = self.decode()#execute decode()
        if line == -1: 
            return False
        else :
