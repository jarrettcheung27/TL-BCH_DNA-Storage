...
class DNAFountain:
    ...
    def __init__(self, 
        ...
        #data:
        self.file_in = file_in #hexadecimal digits
        self.chunk_size = len(file_in[0])
        self.num_chunks = len(file_in)#chunks number of a 
        ...
        #random mnumber generator ?to identify different strands?
        self.lfsr = lfsr(lfsr32s(), lfsr32p()) #starting an lfsr with a certain state and a polynomial for 32bits.#pseudo-random number generator
        
#         print('meaning of self.lfsr: ',self.lfsr)
#         print('datatype of self.lfsr: ',type(self.lfsr))
        self.lfsr_l = len('{0:b}'.format( lfsr32p())) - 1 #calculate the length of lsfr in bits 
        self.seed = self.lfsr.__next__()#
#         print('meaning of self.seed: ',self.seed)
...
#         print('meaning of self.PRNG: ',self.PRNG)
    ...
    def calc_oligo_length(self):
        ...
        return bits/4 #one nucleotides include 4 bits data
    ...
    def screen(self, droplet):
        if self.scanner.Pass(droplet.toDNA()):
            self.good += 1
            dna = droplet.toDNA()#if the droplet satisfy requirement, transform the dropplet to dna
...
class Glass:
    def __init__(self, in_file_name, chunk_num, header_size = 4, 
        ...
        self.rs = rs
        self.RSCodec = None
        self.correct = flag_correct#whether the rs code is used to correct error
        self.seen_seeds = set()#a set is an unordered collection of unique elements,to remove the identical element?
                        #creat an empty set
    ...
    def add_dna(self, dna_string):
        ...
        data = dna_to_int_array(dna_string)#{A,C,G,T} to {00,01,10,11}
        
#         print('Log[decode.Glass.add_dna.dna_string]:',dna_string);
#         print('Log[decode.Glass.add_dna.data]:',data);
        # try error correcting, if rs code is added and we want to correct error
        if self.rs > 0:
            if self.correct: 
                flag, data_corrected = rs_decode(data, self.RSCodec)#flag = 0: code corrected;flag = -1: unable to decode. 
                if flag == -1:
                    return -1, None
        ...
        seed_array = data_corrected[:self.header_size]#取出前面的4byte seeds
#         print('Log[decode.Glass.add_dna.seed_array]:',seed_array);
        seed = sum([   int(x)*256**i        for i, x in enumerate(seed_array[::-1])   ])
#         print('Log[decode.Glass.add_dna.seed]:',seed);
        payload = data_corrected[self.header_size:]#取出中间有效数据的部分
        self.add_seed(seed)#add every unique seed to a set name 'seen_seeds'
...
#         print('Log[decode.Glass.droplet.num_chunks]:',droplet.num_chunks);
        for chunk_num in (droplet.num_chunks & self.done_segments):#to find the common elements between the sets droplet.num_chunks and self.done_segments. droplet.num_chunks：a list of input segment identifiers；done_segments：segments that were already inferred
    ...
    def add_seed(self, seed):
        self.seen_seeds.add(seed)#add the seed into the set, with it cuplicated seed removed
#         print('Log[decode.Glass.add_dna.seen_seeds]:',self.seen_seeds)
...
#         print('Log[DNAFountian.Glass.save.chunks_after_padding]:',self.chunks)
        with open(file_name,'wb') as f: #binary write mode
...
#             logging.info('saved')
    ...
    def decode(self):
        f = open(self.in_file_name,'r')#.dna file after simulation
        line = 0
        errors = 0
        solve_num = []
        while True:
...
#                 print('Log[NDA_fountain.Glass.decode.dna]:',dna)
            except:
...
#             print('Log[decode.Glass.chunksDone.done_segments]:',self.done_segments)
            if seed == -1:
                errors += 1
            #logging
            if line % 200 == 0:
