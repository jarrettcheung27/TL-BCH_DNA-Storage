...
Prob_E = np.linspace(0.068, 0.06, num = 5)
# Prob_E = [0]
seq_depth = 15
r = 117 # Redundancy for index + data
sample_num = 320
# sample_num = 2
##=============File path==================
...
numeric_result_path = 'Data/Simulation Result of LDPC-RS Code for DNA Storage/One Layer Scheme/Simulation-8/Simulation-1.csv'
##=====================Creat file for simulation result========
# Creat a CSV file for simulation result
header = [['r','Seq_depth', 'PE', 'BER', 'FER']]  # Header for the .csv file.
...
n_2 = k_2 + index_length + r  # Codeword length of BCH
print(f'k_1 =  {index_length} bits, k_2 =  {k_2} bits.')
print(f'BCH code argument: n = {n_2}, k = {k_2}, code rate: {(k_2+ index_length)/n_2: .2f}.')
...
for ie, PE in enumerate(Prob_E):
    ...
    ##================Single layer BCH encode in matlab module=================
    print('BCH encoding...')
    # Concatenate index and codeword of outer code.
    msg2 = np.concatenate((ids, np.array(cwr1).T), axis=1) # Message of inner coding
    cwr2 = eng.BCH_Encoder(n_2, k_2 + index_length, n_0, msg2) # Codeword of inner code

    ##===================Convert binary data to DNA=======================
    print('Binary to DNA...')
    dnas = []
    for cw in cwr2:
        dna = bin_to_dna(''.join(str(int(cw[i])) for i in range(n_2)))
        dnas.append(dna)
    ...
    ##===============Single-layer BCH decode=============================
    print('BCH decoding...')
    rx_msgs2 = eng.BCH_Decoder(n_2, k_2 + index_length, len(dnas_sim_result), simu_inf_arr)

    #  Convert double datatype to string.
    rx_ids = []
    rx_cwr1 = []
    for rx_msg in rx_msgs2:
        rx_ids.append(''.join(str(int(s)) for s in rx_msg[1:index_length+1]))
        rx_cwr1.append(''.join(str(int(s)) for s in rx_msg[index_length+1:]))

    ##==========================Sorting==================
    temp = []
    for i, cwr in enumerate(rx_cwr1):
        temp_dict = dict(index=0, num=0, data=' ')
        temp_dict['index'] = int(rx_ids[i], 2)  # Convert binary index to integer index
        temp_dict['data'] = cwr
    ...
    for i in range(n_0):
        ...
        for segment in segments:
            if segment['index'] == i and len(segment['data']) == k_2:
                temp_dict['num'] += 1
                temp_dict['data'].append(segment['data'])
        temp.append(temp_dict)
    segments = temp
