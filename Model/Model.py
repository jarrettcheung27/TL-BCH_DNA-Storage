...
class DNA_Channel_Model:
    ...
    def __call__(self, dnas, inspectFunction = None, print_state = True):
        if print_state: print('Model running...  ')
        ast = time.time()
        for stage_name, module in self.Modules:
            st = time.time()
            if print_state: print(stage_name + ' stage:')
            dnas = module(dnas)
            if print_state: print('Done. Time spent: ' + str(round(time.time() - st,3)) +'s')
            if inspectFunction: inspectFunction(dnas)
        if print_state: print(f'Simulation done.  Total time spent: {round(time.time()-ast,3)}s')
        return dnas
...
class Decayer:
    def __init__(self, arg):
        ...
        # self.err = ErrorAdder(probS = 0, probD = 0, probI = 0, TM = None)
...
class Sequencer:
    def __init__(self,arg):
        self.copies_required = arg.seq_copies#有一个最低的copy要求？
        self.pcrp = arg.seq_prcp
        self.performPCR = arg.seq_performPCR#是否执行pcr扩增
        
        self.seq_depth = arg.seq_depth
        
        self.TM = arg.seq_TM#测序的技术参数？
    ...
    def __call__(self, dnas):
        ...
        if self.TM != 0:
            # self.E = ErrorAdder(probI = 0.00001, probD= 0.00001, TM = self.TM)#引入测序错误
            self.E = ErrorAdder(probI=0, probD=0, TM=self.TM)  # 引入测序错误,只有替代错误
            dnas = self.E(dnas)
        return dnas
    ...
    def pcr(self,dnas):
        ...
        self.pcrc = int(log(amplify_ratio) / log(self.pcrp+1))#自动根据当前copies的数量，确认扩增的循环次数
    ...
    def sample(self, dnas):
        rNs = [dna['num'] for dna in dnas]
        average_copies = sum(rNs) / len(rNs)
        self.sample_ratio = self.seq_depth / average_copies#确保采样后的copies数目就是测序深度
...
class Sampler:
    ...
    def __call__(self,dnas, in_place = False):
        ...
        if self.sam_to_number:
            rNs = [dna['num'] for dna in dnas]
            average_copies = sum(rNs) / len(rNs)
            self.p = self.sam_to_number / average_copies#所以average copies要大于sam_to_number,确保self.p<1
        for counter, dna in enumerate(out_dnas):
            # if counter % 100 == 0:
                # print(f'Progress: {100 * (counter + 1) / len(out_dnas): .2f}%', end='\r')  # Show Progress
...
class PCRer:
    ...
    def __call__(self,dnas,in_place = False):
        ...
        for i, dna in enumerate(out_dnas):
            # if i % 100 == 0:
            #     print(f'Progress: {100 * (i + 1)/len(out_dnas): .2f}%', end = '\r') # Show Progress
            dna['re'] = self.run(dna['re'])
            dna['num'] = sum([tp[0] for tp in dna['re']])
        return out_dnas

class ErrorAdder:
    def __init__(self,probS = 0, probD = 0, probI = 0, TM = None):
        if TM != None: # choose to use TM or set ProbS
            self.TM = TM
            self.all_equal = 0
        else:
            self.TM = genTm(probS)
    ...
    def genNewError(self,dna):
        ...
        for i,base in enumerate(['A','C','G','T']):
            ...
            subi = np.random.choice(['A','C','G','T'],size = Pi.size, p = self.TM[i]) # 选中'A','C','G','T'的概率大小对应与TM[i]中的四个元素。
    ...
    def run(self,ori_dna,re_dnas):
        ...
        new_types = []
        for counter, re_dna in enumerate(re_dnas):
            # print(counter)
